...
class Operations:
    ...
    def replace_ecs_host(self, cluster_name: str, allow_pending=False):
        ...
        if not asg_instance.get("ProtectedFromScaleIn"):
            print(
                f'Instance "{instance_id}" in asg "{asg_name}" does not have scale-in protection, nothing to do..'
            )
        else:
            print(
                f'Removing instance protection from instance "{instance_id}" in asg "{asg_name}"..'
            )
        ...
        self._wait_until_state(
            "ACTIVE", f"ec2InstanceId != {instance_id}", cluster_name
        )
        ...
        print(
            f'Setting instance "{instance_id}" in cluster {cluster_name} to DRAINING..'
        )
        self._ecs.update_container_instances_state(
            cluster=cluster_name, containerInstances=[arn], status="DRAINING"
        )

        # wait til drained, with 0 tasks
        self._wait_until_state(
            "DRAINING", f"ec2InstanceId == {instance_id}", cluster_name, 0
        )

        # verify new one is active
        self._wait_until_state(
            "ACTIVE",
            f"ec2InstanceId != {instance_id}",
            cluster_name,
            target_tasks,
            False,
        )
        ...
        if not self._yes_or_no(
            "new instance active, all tasks drained from old. Remove old instance?"
        ):
            return

        self._autoscale.update_auto_scaling_group(
            AutoScalingGroupName=asg_name, MinSize=1, MaxSize=1, DesiredCapacity=1
        )
    ...
    def _wait_until_state(
        ...
        container_filter,
        cluster_name,
        running_task_target=-1,
        wait_on_first=True,
    ):

        print(
            f'waiting until container matching filter "{container_filter}" reaches "{target_state}"'
        )
        ...
        while not instance_updated:
            ...
            try:
                ...
                instances = self._ecs.list_container_instances(
                    cluster=cluster_name, filter=container_filter
                )
                arns = instances.get("containerInstanceArns")

                container_instances = self._ecs.describe_container_instances(
                    cluster=cluster_name, containerInstances=arns
                )
                ...
                if status != target_state:
                    print(
                        f'Instance still in "{status}" state, wanting "{target_state}"'
                    )
                    continue
                ...
                if running_task_target != -1 and running_tasks != running_task_target:
                    print(
                        f"Instance has {running_tasks} of a targeted {running_task_target} running tasks"
                    )

                    if running_task_target > running_tasks and target_state == "ACTIVE":
                        print(
                            f"Instance is ACTIVE with more tasks that required, marking assuming done"
                        )
